...
-(void)showFile{
    if (self.docURL) {
        self.docInteractionController = [UIDocumentInteractionController interactionControllerWithURL:self.docURL];
...
-(void)openWinPreview:(NSString*)filePath{
    _isShowing = NO;
    self.docURL = [NSURL fileURLWithPath:filePath];
...
- (void)viewDidLoad
{
    ...
    _isShowing=NO;
    CGFloat topY=self.view.frame.size.height-[self topHeight];
    if ([self.fileType isEqualToString:@"4"]) {//档案 
        ...
        if (args.isExistsFileDownload) {//文件已下载
            ...
            if (!_progress) {
                ...
                CGSize textSize=[memo textSize:[UIFont boldSystemFontOfSize:17] withWidth:300];
            ...
            [[PreviewFileManager sharedInstance] startBlockDownLoadFile:args progress:^(ASIHTTPRequest *request){
                ...
                [_labTitle removeFromSuperview];
                _labTitle=nil;
                [AlertHelper initWithTitle:@"提示" message:@"檔案加載失敗!"];
            }];
        }
    }else{
