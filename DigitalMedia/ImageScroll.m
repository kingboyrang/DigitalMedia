...
-(void)reloadScrollView{
    for (int i=0;i<[self.listData count];i++) {
        ...
        [imageView loadImageForURLString:[self.listData objectAtIndex:i]];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(stopScrollWithIndex:)]) {
        [self.delegate stopScrollWithIndex:curPage];
    }
}
...
-(void)imageTouch:(NSSet *)touches withEvent:(UIEvent *)event whichView:(id)imageView{
    ...
    if (total==1) {
        if (self.delegate&&[self.delegate respondsToSelector:@selector(selectedItemWithIndex:sender:)]) {
            [self.delegate selectedItemWithIndex:curPage sender:self];
        }
    }
}
@end
