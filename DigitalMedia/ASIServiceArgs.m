...
static NSString *defaultDataServiceUrl=@"http://webservice.webxml.com.cn/WebServices/ForexRmbRateWebService.asmx";
static NSString *defaultDataServiceNameSpace=defaultWebServiceNameSpace;

@implementation ASIServiceArgs
+(void)setWebServiceURL:(NSString*)url
{
    if (defaultDataServiceUrl!=url) {
        [defaultDataServiceUrl release];
        defaultDataServiceUrl=[url retain];
    }
}
+(void)setNameSapce:(NSString*)space
{
    if (defaultDataServiceNameSpace!=space) {
        [defaultDataServiceNameSpace release];
        defaultDataServiceNameSpace=[space retain];
    }
}
-(id)init{
    if (self=[super init]) {
